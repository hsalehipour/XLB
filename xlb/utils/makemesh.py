...
def generate_mesh(
    ...
    padding_table : dict
        A dictionary specifying padding values for each level to control voxel growth.
        Keys are level numbers (int), values are tuples of six integers (xn, xp, yn, yp, zn, zp)
        representing padding in negative and positive x, y, z directions, respectively.
    ...
    ground_refinement_level : int, optional
        ...
        If None, no ground refinement is performed. Default is None.
...
def makeMesh(levels, filename, voxSize, kernel, domainMultiplier, close=True, ground_refinement_level=None, ground_voxel_height=4, downsample=-1):
    ...
    # domainMin = np.array([partDomain[1][0] - (domainMultiplier['-x'] * partSize[0]),
    # partDomain[1][1] - (domainMultiplier['-y']*partSize[1]),
    # partDomain[1][2] - (domainMultiplier['-z']*partSize[2])
    # ],float)
    domainMax = np.array(
        [
    ...
    print()
    return level_data, domainSize / voxSize
