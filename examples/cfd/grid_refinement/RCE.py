...
u_physical = 40  # Physical inlet velocity in m/s
...
file_output_crossover_percentage = 90.0  # Crossover at 80% of iterations
...
stl_filename = "examples/cfd/stl-files/RCE.stl"
level_data, body_vertices, grid_shape_zip, partSize, actual_num_levels = generate_makemesh_mesh(
    stl_filename,
    voxel_size,
    # ground_refinement_level=None,  # Optional: override default if needed
    # ground_voxel_height=4      # Optional: override default if needed
)

# Characteristic length (body length, approximately 1.044 meters)
L = partSize[0]  # Use x-dimension (length) of the body
...
def print_lift_drag(sim):
    ...
    # Approximate frontal area of body (width * height)
    reference_area = (partSize[1] * partSize[2]) / voxel_size**2
